...
from bfConfig_unicode import bfVersion, langColumns
...
def read_list(fontname, csvFile, alias):
    ...
    status = 0
    try:
        logging.info('readlist %s %s %s',fontname, csvFile, alias)
...
def main(*ffargs):   
    ...
    if len(args) > 2:
        ...
        status = read_list(ttfFont, csvFile, alias)
