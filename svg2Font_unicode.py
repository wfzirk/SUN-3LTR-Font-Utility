...
from bfConfig_unicode import bfVersion, langColumns     #,  readCfg
        ...
        try:
            ...
            if exists:
                ...
                if unicode == -1:
                    ...
                    glyph = font.createChar(int(unicode, 16))
                glyph.importOutlines(file)  
                glyph.glyphname = imagename.replace(' ','_')
                scale(glyph)    #, mn, mx)
                setBearing(glyph)           
            else:
    ...
    try:
        with open(csvFile, encoding='utf8') as csvDataFile:
            ...
            for row in csvReader:
                ...
                addFont(font, unicode, alias, name)
                cnt+=1
...
def convertfiletype(filename):
    fname, lExt = filename.split(".")
    #lang = fname[:3].toupper()
